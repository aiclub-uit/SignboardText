...
def validate_data(gtFilePath, submFilePath,evaluationParams):
    ...
    for k in subm:
        if (k in gt) == False :
            raise Exception(f"The sample {k} not present in GT")
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    def get_union(pD,pG):
        ...
        res = areaA + areaB - intersection
        # convert pD and pG to list of points
        # pD_list = pD.exterior
        # print(f"pD {pD}, pG {pG}, intersection {intersection}, res: {res}")
        return res
    ...
    debug_file = "0000001.txt"
    debug = False
    for resFile in gt:
        if resFile != debug_file and debug: continue
        ...
        if resFile == debug_file and debug:
            print(f"{resFile}, {recall}, {precision}, {sampleAP}")
            print(len(gtPols), len(detPols), len(gtDontCarePolsNum), len(detDontCarePolsNum))
            print(iouMat)
            print(gtRectMat)
            print(detRectMat)
            print(arrGlobalMatches)
            print(arrSampleMatch)
    ...
    methodPrecision = f"{methodPrecision*100:.2f}"
    methodRecall = f"{methodRecall*100:.2f}"
    methodHmean = f"{methodHmean*100:.2f}"
