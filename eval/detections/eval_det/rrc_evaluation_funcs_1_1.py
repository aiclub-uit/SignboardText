...
file_handler = logging.FileHandler('rrc_evaluation_funcs_1_1.log')    

def print_help():
    print('Usage: python %s.py -g=<gtFile> -s=<submFile> [-o=<outputFolder> -p=<jsonParams>]' %sys.argv[0])
    sys.exit(2)
...
def validate_lines_in_file(fileName,file_contents,CRLF=True,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0):
    ...
    if (utf8File is None) :
        raise Exception(f"The file {fileName} is not UTF-8")
    ...
    for line in lines:
        ...
        if(line != ""):
            try:
                ...
                raise Exception((f"Line in sample not valid. Sample: {fileName} Line: {line} Error: {str(e)}").encode('utf-8', 'replace'))
...
def get_tl_line_values(line,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0):
    ...
    if LTRB:
        ...
        ymax = int(m.group(4))
        if(xmax<xmin):
                raise Exception(f"xmax:{xmax} xmin:{xmin} value not valid (xmax < xmin).")
        if(ymax<ymin):
                raise Exception(f"ymax:{ymax} ymin:{ymin} value not valid (ymax < Ymin).")  
        ...
        # logger.info(withTranscription)
        # logger.info(withConfidence)
        if withTranscription and withConfidence:
